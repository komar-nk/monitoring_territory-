...
class NotificationManager:
    ...
    def send_change_notification(self, territory_info: Dict[str, Any],
                                 ...
                                 grid_comparison_path: Optional[str] = None) -> bool:
        """
        ...
        # ВСЕГДА используем обычный метод отправки (у тебя он есть)
    ...
    def _create_comparison_image(self, old_path: str, new_path: str,
        ...
        try:
            ...
            if old_date:
                cv2.putText(comparison, f"Дата: {old_date}", (10, height + 60),
                            font, 0.5, (200, 200, 255), 1)
            if new_date:
                cv2.putText(comparison, f"Дата: {new_date}", (width + 10, height + 60),
                            font, 0.5, (200, 200, 255), 1)
...
<head>
    ...
    <style>
        ...
        .percentage {{ font-size: 28px; font-weight: bold; color: {color}; text-align: center; margin: 20px 0; }}
...
<body>
    <div class="container">
        ...
        <div class="percentage">{change_percent:.1f}% изменений</div>
    ...
    def _attach_files(self, msg: MIMEMultipart, files_info: Dict[str, Dict]) -> int:
        ...
        for file_type, info in files_info.items():
            ...
            try:
                ...
                print(f"  ⚠️ Ошибка прикрепления {file_path}: {e}")
    ...
    def __init__(self, dotenv_file: str = '.env'):
        try:
            ...
            if self.EMAIL_ENABLED and self.EMAIL_FROM and self.EMAIL_PASSWORD:
                ...
                print(f"⚠️ Конфигурация email неполная или отключена")
